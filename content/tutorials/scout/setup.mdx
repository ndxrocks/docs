...
<Tabs>
  <Tab title="Manual">
    <Steps>
      <Step title="Clone and configure">
        ```bash
        git clone https://github.com/kern-agi/scout && cd scout
        cp example.env .env
        ```
      ...
      <Step title="Verify it's running">
        ...
        Expect `{"status":"ok"}`. Connection refused means the container is still starting. Check `docker compose logs -f scout-api` and wait for the Kern banner.
      </Step>
    </Steps>
  </Tab>
  ...
  <Tab title="Agentic">
    ...
    Steps:
    1. Clone https://github.com/kern-agi/scout into the current directory and cd into it.
...
| **Workspace** | `query_workspace` | The Scout repo, read-only. Re-point at your own repo by editing `SCOUT_FS_ROOT` in [`scout/contexts.py`](https://github.com/kern-agi/scout/blob/main/scout/contexts.py). |
