...
1. Open [os.kern.ndx.rocks](https://os.kern.ndx.rocks), click **Add OS** → **Live**, and enter your Railway domain.
...
The Kern control plane handles JWT issuance, session management, traces, metrics, and the web UI. Scout just verifies the JWTs it sees. See the [AgentOS security docs](/agent-os/security/overview) for the full picture.

<Warning>
  Opting out is not recommended. If you must run production without auth (e.g. inside a private VPC behind another auth layer), flip `authorization=False` in [`app/main.py`](https://github.com/kern-agi/scout/blob/main/app/main.py) before deploying. Without it, anyone who guesses your Railway domain can query your CRM, wiki, and connected sources.
</Warning>
...
The OS you added in Step 2 should now show as connected at [os.kern.ndx.rocks](https://os.kern.ndx.rocks).
...
Every `update_knowledge` call commits with an LLM-summarised one-line message and pushes. `git log -p wiki/knowledge/` is your audit trail. Full setup: [WIKI_GIT.md](https://github.com/kern-agi/scout/blob/main/docs/WIKI_GIT.md).
...
Scale CPU, memory, or replicas in [`railway.json`](https://github.com/kern-agi/scout/blob/main/railway.json).
