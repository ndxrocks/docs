...
The full reference, including transport details and the Node-in-Docker gotcha, lives in the Scout repo: [MCP_CONNECT.md](https://github.com/kern-agi/scout/blob/main/docs/MCP_CONNECT.md).

## Wire up a server

Add an `MCPContextProvider` entry to `_create_mcp_providers()` in [`scout/contexts.py`](https://github.com/kern-agi/scout/blob/main/scout/contexts.py). Secrets come from the process env via `getenv(...)`.
...
The [`kern.context.web.provider`](https://github.com/kern-ai/kern/blob/main/libs/kern/kern/context/web/provider.py) module is a complete reference implementation.
...
For more, see [MCP_CONNECT.md](https://github.com/kern-agi/scout/blob/main/docs/MCP_CONNECT.md).
