...
<Steps>
  <Step title="Clone the template">
    ```bash
    git clone https://github.com/kern-agi/agentos-railway-template.git agent-platform
    cd agent-platform
    ```
  ...
  <Step title="Verify it's running">
    ...
    Expect `{"status":"ok"}`. If you get connection refused, the container is still starting; check `docker compose logs -f` and wait for the Kern banner.
