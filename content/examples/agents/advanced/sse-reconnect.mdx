...
description: "Reconnect to a background agent stream after disconnection using the /resume endpoint."
---
...
1. An [AgentOS server](/agent-os/run-your-os) running on `http://localhost:7777` with at least one agent registered (e.g., `python cookbook/05_agent_os/basic.py`).
...
uv pip install -U kern-ai httpx
