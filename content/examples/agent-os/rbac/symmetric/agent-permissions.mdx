...
description: "This example demonstrates how to define per-agent permission scopes to control which users can run which specific agents."
---
```python
"""
...
if __name__ == "__main__":
    ...
    - "agents:kern-agent:run" - Run only the kern-agent
    ...
    print("\nAgno User Token (agents:kern-agent:run):")
    ...
    print(
        '\ncurl -H "Authorization: Bearer '
        + agno_user_token
        + '" http://localhost:7777/agents/kern-agent/runs'
    )
    print("\n" + "=" * 60 + "\n")
...
cd kern/cookbook/05_agent_os/rbac/symmetric
