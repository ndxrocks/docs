---
title: "Example demonstrating how to connect to a remote Kern A2A agent."
sidebarTitle: "Remote Kern A2A Agent"
description: "This example shows how to use RemoteAgent with the A2A protocol to connect to an Kern agent that's exposed via the A2A interface."
---
```python
"""
Example demonstrating how to connect to a remote Kern A2A agent.

This example shows how to use RemoteAgent with the A2A protocol to connect
to an Kern agent that's exposed via the A2A interface.

Prerequisites:
1. Start an Kern A2A server:
...
async def remote_agno_a2a_agent_example():
    """Call a remote Kern agent exposed via A2A interface."""
    # Connect to remote Kern A2A agent
    # protocol="a2a" tells RemoteAgent to use A2A protocol
    # a2a_protocol="rest" uses REST API (default for Kern A2A servers)
    agent = RemoteAgent(
    ...
    print("Calling remote Kern A2A agent...")
...
async def remote_agno_a2a_streaming_example():
    """Stream responses from a remote Kern A2A agent."""
    agent = RemoteAgent(
    ...
    print("\nStreaming response from remote Kern A2A agent...")
...
async def remote_agno_a2a_agent_info_example():
    """Get information about a remote Kern A2A agent."""
    agent = RemoteAgent(
        base_url="http://localhost:7779/a2a/agents/assistant-agent-2",
...
async def main():
    ...
    print("=" * 60)
    print("Remote Kern A2A Agent Examples")
    print("=" * 60)
    print("\nNote: Make sure the Kern A2A server is running on port 7779")
    print("Start it with: python cookbook/05_agent_os/remote/agno_a2a_server.py\n")

    # Run examples
    print("1. Remote Kern A2A Agent Example:")
    await remote_agno_a2a_agent_example()

    print("\n2. Remote Kern A2A Streaming Example:")
    await remote_agno_a2a_streaming_example()

    print("\n3. Remote Kern A2A Agent Info Example:")
