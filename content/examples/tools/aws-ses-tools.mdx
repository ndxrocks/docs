---
title: "AWS SES"
description: "Use AWS Simple Email Service (SES) with Kern Agents."
---

Enable Kern agents programmatically send and manage emails with AWS SES.
...
from kern.tools.aws_ses import AWSSESTool
...
agent = Agent(
    ...
    instructions=[
        ...
        "5. Send the email using AWS SES. IMPORTANT: The receiver_email parameter must be the COMPLETE email address including the @ symbol and domain (e.g., if the user says 'send to mustafa@kern.ndx.rocks', you must use receiver_email='mustafa@kern.ndx.rocks', NOT 'mustafacom' or any other variation).",
    ],
    tools=[
        AWSSESTool(
...
if __name__ == "__main__":
    agent.print_response(
        "Research AI developments in healthcare from the past week with a focus on practical applications in clinical settings. Send the summary via email to mustafa@kern.ndx.rocks"
    )
...
```
For details, see [AWS SES cookbook](https://github.com/kern-ai/kern/blob/main/cookbook/91_tools/aws_ses_tools.py).