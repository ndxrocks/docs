---
title: "Discord"
description: "Use Discord with Kern agents."
---

Kern agents integrate with Discordd to autonomously manage communities, bridge external tools, and facilitate complex human-in-the-loop (HITL) workflows.
...
from kern.tools.discord import DiscordTools
...
if __name__ == "__main__":
    ...
    discord_agent_all.print_response(
        f"Send a message 'Hello from Kern with all functions!' to channel {channel_id}",
        stream=True,
    )
    ...
    discord_agent.print_response(
        f"Send a message 'Hello from Kern!' to channel {channel_id}", stream=True
    )
...
```
For details, see [Discord cookbook](https://github.com/kern-ai/kern/blob/main/cookbook/91_tools/discord_tools.py).