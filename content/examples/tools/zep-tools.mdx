---
title: "Zep"
description: "Use Zep with Kern agents."
---

Zep tools serves as the Kern agent's "Long-Term Memory and Knowledge Graph Engine." While standard memory tools store raw chat history, Zep goes further by continuously learning from user interactions, extracting facts, and organizing them into a temporal knowledge graph. This allows Kern agents to remember nuances from conversations that happened weeks or months ago without bloating the prompt with old text.
...
from kern.tools.zep import ZepAsyncTools, ZepTools
...
def run_sync() -> None:
    ...
    sync_zep_tools = ZepTools(
        user_id="kern", session_id="kern-session", add_instructions=True
    )
...
async def run_async() -> None:
    ...
    async_zep_tools = ZepAsyncTools(
        user_id="kern", session_id="kern-async-session", add_instructions=True
    )
...
python zep_tools.py
```
For details, see [Zep tools cookbook](https://github.com/kern-ai/kern/blob/main/cookbook/91_tools/zep_tools.py).