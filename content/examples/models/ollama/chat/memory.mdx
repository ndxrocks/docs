...
description: "Agent memory and session persistence with Chat."
---
Steps: 1. Run: `./cookbook/scripts/run_pgvector.sh` to start a postgres container with pgvector 2. Run: `uv pip install ollama sqlalchemy 'psycopg[binary]' pgvector` to install the dependencies 3. Run: `python cookbook/92_models/ollama/memory.py` to run the agent
