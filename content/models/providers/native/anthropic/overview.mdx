...
description: Use Anthropic Claude models with Kern agents.
---
...
Read more about beta features with Kern `Claude` model [here](/models/providers/native/anthropic/usage/betas).
...
Read more about prompt caching with Kern's `Claude` model [here](/models/providers/native/anthropic/usage/prompt-caching).
...
Kern uses Claude's native support for structured outputs. This feature is available for `claude-sonnet-4-5-20250929` and all newer models. See Anthropic's [structured outputs documentation](https://docs.anthropic.com/en/build-with-claude/structured-outputs) for more details.
