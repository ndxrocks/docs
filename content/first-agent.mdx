...
keywords: [kern, agent, quickstart, tutorial]
---
...
1. **Add session storage**, so we can have a conversation with our agent. Kern automatically manages session read, write and context injection for you.
2. **Add memory**, so our agent learns from usage patterns. Kern automatically handles memory management and exposes an `update_user_memory` tool to the agent.
...
AgentOS also comes with a UI, available at: [os.kern.ndx.rocks](https://os.kern.ndx.rocks). It connects directly from your browser to the running API. Use it to test, monitor, and manage your agents in real time.
...
- [Wire up kern with your favorite coding agent. →](/coding-agents)
