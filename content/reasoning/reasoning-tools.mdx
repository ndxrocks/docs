...
```python
from kern.agent import Agent
from kern.knowledge.pdf import PDFKnowledgeBase
from kern.models.openai import OpenAIResponses
...
- `ReasoningTools`, `KnowledgeTools`, `MemoryTools`, and `WorkflowTools` all include Kern-authored instructions (and optional few-shot examples) describing their Think → Act → Analyze workflow.
