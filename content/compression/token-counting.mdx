...
When you set `compress_token_limit`, Kern checks the estimated token count during the run loop and triggers compression when the threshold is reached.
...
Kern uses conservative estimates for multimodal inputs to support context planning:
