...
Kern gives you the orchestration primitives. These concerns are yours to add, and the cookbook is explicit about each.

| Concern | What to add |
|---------|-------------|
| Rate limiting | Wrap `agent.arun` with a per-provider limiter, or front it with a gateway. Kern does not throttle outbound calls. |
...
| Provider Batch APIs | For non-urgent jobs, call the provider batch endpoints directly for the discount. Not wrapped by Kern. |
...
- [Quality review cookbook](https://github.com/kern-ai/kern/tree/main/cookbook/data_labeling/_18_quality_review)
