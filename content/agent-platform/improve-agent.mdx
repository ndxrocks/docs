...
Claude asks what to change. You describe a tool to add, a prompt to refine, a bug to fix. The kern-docs MCP is loaded so toolkit research is grounded in the real API. Each iteration is one small, verified change.
