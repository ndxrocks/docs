---
title: AgentQL
---

**AgentQLTools** enable an Agent to browse and scrape websites using the AgentQL API.

## Prerequisites

The following example requires the `agentql` library and an API token which can be obtained from [AgentQL](https://agentql.com/).

```shell
uv pip install -U agentql
```

```shell
export AGENTQL_API_KEY=***
```

## Example

The following agent will open a web browser and scrape all the text from the page.

```python cookbook/14_tools/agentql_tools.py
from kern.agent import Agent
from kern.models.openai import OpenAIResponses
from kern.tools.agentql import AgentQLTools

agent = Agent(
    model=OpenAIResponses(id="gpt-5.2"), tools=[AgentQLTools()]
)

agent.print_response("https://kern.ndx.rocks/introduction", markdown=True)
```

<Note>
  AgentQL will open up a browser instance (don't close it) and do scraping on
  the site.
</Note>

## Toolkit Params

| Parameter                      | Type   | Default | Description                                       |
| ------------------------------ | ------ | ------- | ------------------------------------------------- |
| `api_key`                      | `str`  | `None`  | API key for AgentQL                               |
| `enable_scrape_website`        | `bool` | `True`  | Enables functionality to scrape websites         |
| `enable_custom_scrape_website` | `bool` | `False` | Enables functionality to use custom AgentQL queries |
| `all`                          | `bool` | `False` | Enables all functionality when set to True       |
| `agentql_query`                | `str`  | `""`    | Custom AgentQL query string                       |

## Toolkit Functions

| Function                | Description                                          |
| ----------------------- | ---------------------------------------------------- |
| `scrape_website`        | Used to scrape all text from a web page              |
| `custom_scrape_website` | Uses the custom `agentql_query` to scrape a web page |

## Developer Resources

- View [Tools](https://github.com/kern-ai/kern/blob/main/libs/kern/kern/tools/agentql.py)
