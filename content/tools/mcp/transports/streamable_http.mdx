...
mcp_tools = MCPTools(url="https://kern.ndx.rocks/mcp", transport="streamable-http")
await mcp_tools.connect()

try:
    agent = Agent(model=OpenAIResponses(id="gpt-5.2"), tools=[mcp_tools])
    await agent.aprint_response("What can you tell me about MCP support in Kern?", stream=True)
finally:
...
```python
from kern.tools.mcp import MCPTools, StreamableHTTPClientParams
