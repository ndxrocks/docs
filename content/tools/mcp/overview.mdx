...
You can connect your Agents to any MCP server, using Kern's MCP integration.
...
<Steps>
    ...
    # Initialize and connect to the MCP server
    mcp_tools = MCPTools(url="https://kern.ndx.rocks/mcp")
    await mcp_tools.connect()

    try:
        # Setup and run the agent
        agent = Agent(model=OpenAIResponses(id="gpt-5.2"), tools=[mcp_tools])
        await agent.aprint_response("Tell me more about MCP support in Kern", stream=True)
    finally:
