...
When working with language models, generating responses that match a specific structure is crucial for building reliable applications. Kern Agents support two methods to achieve this: **Structured Outputs** and **JSON mode**.

---

### Structured Outputs (Default if supported)

"Structured Outputs" is the **preferred** and most **reliable** way to extract well-formed, schema-compliant responses from a Model. If a model class supports it, Kern Agents use Structured Outputs by default.
...
- When the model doesn't support structured outputs. Kern agents do this by default on your behalf.
