---
title: Migrating to Kern v2.0
sidebarTitle: Kern v2.0 Migration Guide
description: Guide to migrate your Kern applications from v1 to v2.
---
...
Otherwise, you can install the latest version of Kern v2 by running:
```bash
pip install kern-ai
```

## Migrating your Kern DB
...
Use our migration script: [`libs/kern/migrations/v1_to_v2/migrate_to_v2.py`](https://github.com/kern-ai/kern/blob/main/libs/kern/migrations/v1_to_v2/migrate_to_v2.py)
...
[Agents](/agents/overview) and [Teams](/teams/overview) are the main building blocks in the Kern framework.
...
from kern.agent import Agent
from kern.storage.sqlite import SqliteStorage

storage = SqliteStorage(table_name="agent_sessions", db_file="kern.db", mode="agent")
...
db = SqliteDb(db_file="kern.db", sessions_table="your_sessions_table_name", ...)
```
...
```python v1_memory.py
from kern.agent import Agent
from kern.memory.v2.db.sqlite import SqliteMemoryDb
from kern.memory.v2.memory import Memory

memory_db = SqliteMemoryDb(table_name="memory", db_file="kern.db")
...
db = SqliteDb(db_file="kern.db", memory_table="your_memory_table_name")
```
...
```python
from kern.app.agui.app import AGUIApp
