...
sidebarTitle: Kern v2.0 Changelog
---
...
This is a major rewrite of the Kern library and introduces various new concepts and updates to the existing ones.
...
<Accordion title="Repo Updates">
  - `/libs/kern` has been restructured to fit the new concepts in Kern and for better organization.
  - All code related to managing workspaces and agent deployment in Kern has been moved to a new package called `kern-infra`. This is a combination of the previous `kern-aws` and `kern-docker` packages, as well as the CLI and other tools.
  - `kern-aws` and `kern-docker` packages have been deprecated and will no-longer be maintained.
  - All code related to the Kern CLI (`ag`) has been moved to this new `kern-infra` package.
  - Added `AgentOS` to `kern` as a comprehensive API solution for building multi-agent systems. This also replaces `Playground` and other Apps. See details below.
...
<Accordion title="AgentOS">
  ...
  - This enables you to host agents, teams and workflows, and use the [Agent OS UI](https://os.kern.ndx.rocks) to manage them.
</Accordion>
...
<Accordion title="Updates to Run Objects">
  ...
  - The import location for `RunOutput` (and events) has been moved to `kern.run.agent`.
...
- Import locations for `embedder`, `document`, `chunking`, `reranker` and `reader` have been moved to `kern.knowledge`. See [examples](/knowledge/overview) for more details.
...
<Accordion title="Updates to Agent Class">
  ...
  - Removed all `monitoring` from `Agent`. With the new AgentOS platform, monitoring is done using your own data. Go to [os.kern.ndx.rocks](https://os.kern.ndx.rocks) to get started.
</Accordion>
...
<Accordion title="Updates to Team Class">
  ...
  - Removed all `monitoring` from `Team`. With the new AgentOS platform, monitoring is done using your own data. Go to [os.kern.ndx.rocks](https://os.kern.ndx.rocks) to get started.
</Accordion>
...
<Accordion title="Updates to Workflow Class">
  ...
  - This means the import locations for "Workflows v2" is now `kern.workflows`.
  - `extra_data` -> `metadata`
  ...
  - Removed all `monitoring` from `Workflow`. With the new AgentOS platform, monitoring is done using your own data. Go to [os.kern.ndx.rocks](https://os.kern.ndx.rocks) to get started.
</Accordion>
