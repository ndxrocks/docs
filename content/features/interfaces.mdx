...
| **Discord** | Community servers, gaming, custom commands. Runs in its own process via `kern.integrations.discord`. | [Discord](/deploy/interfaces/discord/overview) |
...
The [Scout](/tutorials/scout/connect-slack), [Dash](/tutorials/dash/connect-slack), and [Coda](/tutorials/coda/connect-slack) tutorials use this pattern. Slack only loads when both env vars are set, so dev runs without credentials don't crash.

## Custom interfaces and one-off webhooks

The interface API is small. Subclass `BaseInterface`, register routes on the FastAPI app via `setup`, dispatch incoming messages to the agent. See [BaseInterface](https://github.com/kern-ai/kern/blob/main/libs/kern/kern/os/interfaces/base.py) for the full surface.
