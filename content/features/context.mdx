...
An agent is only as good as the context it has access to. Even a perfect system prompt fails without the right context at runtime.
...
from kern.vector_db.pgvector import PgVector
...
```python
from kern.agent import Agent
from kern.context.slack import SlackContextProvider
from kern.context.gdrive import GDriveContextProvider
from kern.context.database import DatabaseContextProvider
...
```python
from kern.context.web import WebContextProvider, ExaBackend
...
```python
from kern.context.mode import ContextMode
from kern.context.web import WebContextProvider, ExaBackend
...
```python
from kern.context import Answer, ContextProvider, Status

FAQ = {"pricing": "See kern.ndx.rocks/pricing", "support": "Email help@kern.ndx.rocks"}
...
Kern includes four Google-specific providers. All support two auth methods:
...
<Tabs>
  <Tab title="Gmail">
    ...
    ```python
    from kern.context.gmail import GmailContextProvider
  ...
  <Tab title="Calendar">
    ...
    ```python
    from kern.context.calendar import GoogleCalendarContextProvider

    calendar = GoogleCalendarContextProvider()

    # Read-only: agent can check availability but not create events
    calendar = GoogleCalendarContextProvider(write=False)
    ```
  ...
  <Tab title="Drive">
    ...
    ```python
    from kern.context.gdrive import GoogleDriveContextProvider
  ...
  <Tab title="Workspace">
    ...
    ```python
    from kern.context.workspace import WorkspaceContextProvider
...
<Tabs>
  <Tab title="Filesystem">
    ...
    ```python
    from kern.context.wiki import WikiContextProvider
    from kern.context.wiki.backend import FileSystemBackend
  ...
  <Tab title="Git">
    ...
    ```python
    from kern.context.wiki import WikiContextProvider
    from kern.context.wiki.backend import GitBackend
...
<Accordion title="Optional: Web ingestion">
  ...
  ```python
  from kern.context.web import ExaBackend
...
<Card title="Scout" icon="rocket" href="/tutorials/scout/overview">
...
<CardGroup cols={2}>
  <Card title="Filesystem" icon="folder" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/00_filesystem.py">
    Agent explores your codebase or docs folder
  </Card>
  <Card title="Database" icon="database" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/04_database_read_write.py">
    Read and write with separate engines for safety
  </Card>
  <Card title="MCP Server" icon="plug" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/06_mcp_server.py">
    Raw MCP tools on the agent, no sub-agent wrapper
  </Card>
  <Card title="Multi-provider" icon="layer-group" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/08_multi_provider.py">
    Three sources on one agent, no tool collisions
  </Card>
  <Card title="Web + Slack" icon="arrows-split-up-and-left" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/09_web_plus_slack.py">
    Pull topics from Slack, research on web, synthesize
  </Card>
  <Card title="Custom provider" icon="code" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/10_custom_provider.py">
    Build your own provider for any data source
  </Card>
  <Card title="Gmail" icon="envelope" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/18_gmail.py">
    Search, read, draft, send emails
  </Card>
  <Card title="Calendar" icon="calendar" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/19_calendar.py">
    Query events, check availability, create meetings
  </Card>
  <Card title="Wiki (filesystem)" icon="book" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/14_wiki_filesystem.py">
    Prose memory backed by local markdown files
  </Card>
  <Card title="Wiki (git)" icon="code-branch" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/15_wiki_git.py">
    Prose memory with automatic git commits
  </Card>
  <Card title="Google Workspace" icon="google" href="https://github.com/kern-ai/kern/blob/main/cookbook/12_context/20_google_workspace.py">
