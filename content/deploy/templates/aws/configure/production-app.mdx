...
Your application uses the `kern` images by default. To use your own image:
...
The repo name should match the `image_name` in `settings.py` (default: `agentos-aws-template`).
...
- If using dockerhub it would be something like `kern`.
