...
Kern templates come pre-configured with [SqlAlchemy](https://www.sqlalchemy.org/) and [Alembic](https://alembic.sqlalchemy.org/en/latest/) to manage databases. You can use these tables to store data for your Agents, Teams and Workflows. The general way to add a table is:
...
```bash
docker exec -it {infra_name}-api alembic -c db/alembic.ini revision --autogenerate -m "Initialize DB"
```
...
```bash
docker exec -it {infra_name}-api alembic -c db/alembic.ini upgrade head
```
...
```bash
docker exec -it {infra_name}-api python db/tables/test_add_user.py
```
...
```bash
docker exec -it {infra_name}-api sh -c "cd db && alembic init migrations"
```
