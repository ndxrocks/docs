...
description: Use filter expressions through the Kern API for precise knowledge base filtering.
...
Kern supports two ways to filter knowledge through the API:
...
# Simple dict filter
filter_dict = {"docs": "kern", "status": "published"}
...
response = requests.post(
    "http://localhost:7777/agents/kern-knowledge-agent/runs",
    data={
        "message": "What are kern's key features?",
        "stream": "false",
...
curl -X 'POST' \
  ...
  -F 'message=What are kern'\''s key features?' \
  -F 'stream=false' \
  -F 'session_id=' \
  -F 'user_id=' \
  -F 'knowledge_filters={"docs": "kern"}'
```
...
```python Python Client
from kern.filters import EQ, GT
