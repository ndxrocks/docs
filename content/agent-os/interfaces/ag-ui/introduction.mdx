---
title: AG-UI
description: Expose Kern agents via the AG-UI protocol
---
...
3. Build the Kern package in `/ag-ui/integrations/kern`: `pnpm run build`
4. Start Dojo following the instructions in the repository.

  </Step>
  <Step title="Chat with the Kern Agent">
...
Additional examples are available in the [cookbook](https://github.com/kern-ai/kern/tree/main/cookbook/05_agent_os/interfaces/agui/).
...
- `AGUI` (interface): Wraps an Kern `Agent` or `Team` into an AG-UI compatible FastAPI router.
...
See [cookbook examples](https://github.com/kern-ai/kern/tree/main/cookbook/05_agent_os/interfaces/agui/) for updated interface patterns.
