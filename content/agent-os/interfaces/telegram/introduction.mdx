...
The Telegram interface exposes an Kern Agent, Team, or Workflow on Telegram via FastAPI webhook endpoints. It handles inbound messages (text, photos, audio, video, documents, stickers) and streams responses back to the originating chat.
...
```python
from kern.tools.telegram import TelegramTools
