...
An inner workflow can use the same primitives and combinations as any top-level workflow in Kern: agents, executors, nested `Step`s, `Condition`, `Loop`, `Router`, and `Parallel`, mixed however your pipeline needs.
...
from kern.workflow import Parallel
