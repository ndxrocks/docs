...
The [Taskboard agent](https://github.com/kern-agi/demo-os/blob/main/agents/taskboard/agent.py) is a working task list that lives in one session. State persists across messages in the session. The agent updates it directly.
...
`session_state` gets injected into tool functions automatically (same way `RunContext` does for [Injector](/demo-os/dependencies)). Mutations persist.
...
Source: [`agents/taskboard/agent.py`](https://github.com/kern-agi/demo-os/blob/main/agents/taskboard/agent.py)
