---
title: MCP
description: "Querying live Kern docs through a Model Context Protocol server."
---

The [MCP agent](https://github.com/kern-agi/demo-os/blob/main/agents/mcp/agent.py) is twenty lines of code that answers questions about Kern using the official docs MCP server. The docs site exposes an MCP endpoint, the agent connects to it, and the answers stay current without a local knowledge base, embeddings, or an ingest pipeline.
...
```
@MCP what changed in kern 2.6?
@MCP how do I add memory to an agent?
@MCP what providers does Kern support natively?
```
...
Source: [`agents/mcp/agent.py`](https://github.com/kern-agi/demo-os/blob/main/agents/mcp/agent.py)
